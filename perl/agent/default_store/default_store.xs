...
static double
constant_NETSNMP_DS_AGENT_M(char *name, int len, int arg)
{
    if (18 + 12 >= len ) {
	errno = EINVAL;
	return 0;
    ...
    switch (name[18 + 12]) {
    case 'P':
	if (strEQ(name + 18, "AX_GETBULKREPEATS")) {	/* NETSNMP_DS_AGENT_M removed */
#ifdef NETSNMP_DS_AGENT_MAX_GETBULKREPEATS
	    return NETSNMP_DS_AGENT_MAX_GETBULKREPEATS;
#else
	    goto not_there;
#endif
	}
    case 'S':
	if (strEQ(name + 18, "AX_GETBULKRESPONSES")) {	/* NETSNMP_DS_AGENT_M removed */
#ifdef NETSNMP_DS_AGENT_MAX_GETBULKRESPONSES
	    return NETSNMP_DS_AGENT_MAX_GETBULKRESPONSES;
#else
	    goto not_there;
#endif
	}
...
constant(char *name, int len, int arg)
{
	...
	if (!strnEQ(name + 0,"NETSNMP_DS_AGENT_", 17))
	    break;
	return constant_NETSNMP_DS_AGENT_M(name, len, arg);
    case 'N':
