...
/** Remove socket fd from the set *fdset. Do nothing if not present. */
...
#define NETSNMP_LARGE_FD_ZERO(fdset) \
    ...
    void            netsnmp_large_fd_setfd(SOCKET fd,
                                           netsnmp_large_fd_set * fdset);
    void            netsnmp_large_fd_clr(SOCKET fd,
                                         netsnmp_large_fd_set * fdset);
    int             netsnmp_large_fd_is_set(SOCKET fd,
...
#define NETSNMP_LARGE_FD_ZERO(fdset)                       \
  do {                                                     \
    ...
    void            netsnmp_large_fd_setfd(int fd,
                                           netsnmp_large_fd_set * fdset);
    void            netsnmp_large_fd_clr(int fd,
                                         netsnmp_large_fd_set * fdset);
    int             netsnmp_large_fd_is_set(int fd,
                                            netsnmp_large_fd_set
                                            * fdset);
     ...
     */
    void            netsnmp_large_fd_set_resize(netsnmp_large_fd_set *
                                                fdset, int setsize);

    /** Deallocate the memory allocated by netsnmp_large_fd_set_init. */
    void            netsnmp_large_fd_set_cleanup(netsnmp_large_fd_set *
                                                 fdset);
