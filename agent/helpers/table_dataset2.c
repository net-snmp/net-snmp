...
typedef struct data_set_tables_s {
    netsnmp_table_data2_set *table_set2;
} data_set_tables;
...
/** @defgroup table_dataset2 table_dataset2: Helps you implement a table with automatted storage.
 *  @ingroup table_data2
 *
...
void
netsnmp_init_table_dataset2(void) {
}

/** Create a netsnmp_table_data2_set structure given a table_data2 definition */
netsnmp_table_data2_set *
netsnmp_create_table_data2_set(const char *table_name)
{
    netsnmp_table_data2_set *table_set2 =
...
/** Given a netsnmp_table_data2_set definition, create a handler for it */
netsnmp_mib_handler *
netsnmp_get_table_data2_set_handler(netsnmp_table_data2_set *data_set)
{
    netsnmp_mib_handler *ret = NULL;

    if (!data_set) {
        snmp_log(LOG_INFO,
                 "netsnmp_get_table_data2_set_handler(NULL) called\n");
        return NULL;
    }

    ret =
        netsnmp_create_handler(TABLE_DATA_SET2_NAME,
                               netsnmp_table_data2_set_helper_handler);
    if (ret) {
...
*/
int
netsnmp_register_table_data2_set(netsnmp_handler_registration *reginfo,
                                netsnmp_table_data2_set *data_set,
                                ...
                                *table_info)
{
    ...
    if ((!table_info->min_column || !table_info->max_column) &&
        ...
        netsnmp_table_data2_set_storage *row;
...
netsnmp_table_data2_set_find_column(netsnmp_table_data2_set_storage *start,
                                   unsigned int column)
{
...
/**
 * extracts a netsnmp_table_data2_set pointer from a given request
 */
netsnmp_table_data2_set_storage *
netsnmp_extract_table_data2_set_column(netsnmp_request_info *request,
                                     unsigned int column)
{
...
{
    return (netsnmp_table_data2_set *)
        netsnmp_request_get_list_data(request, TABLE_DATA_SET2_NAME);
}
                                 ...
                                 int writable)
{
    ...
    if (!data) {
         ...
         * create it 
         */
        data = SNMP_MALLOC_TYPEDEF(netsnmp_table_data2_set_storage);
        if (!data) {
            snmp_log(LOG_CRIT, "no memory in netsnmp_set_data2_row_column");
...
/** adds a new default row to a table_set2.
 * Arguments should be the table_set2, column number, variable type and
 ...
 */
int
netsnmp_table_set2_add_default_row(netsnmp_table_data2_set *table_set2,
                                  unsigned int column,
                                  ...
                                  size_t default_value_len)
{
    ...
    netsnmp_table_data2_set_storage *new_col, *ptr, *pptr;
     ...
     */
    new_col =
        netsnmp_table_data2_set_find_column(table_set2->default_row, column);
    if (new_col != NULL) {
    ...
    new_col = SNMP_MALLOC_TYPEDEF(netsnmp_table_data2_set_storage);
    new_col->type = type;
    ...
    if (table_set2->default_row == NULL)
        table_set2->default_row = new_col;
    else {
        /* sort in order just because (needed for add_row support) */
        for (ptr = table_set2->default_row, pptr = NULL;
             ptr;
        ...
        if (pptr)
            ...
            snmp_log(LOG_ERR,"Shouldn't have gotten here: table_dataset2/add_row");
    }
    return SNMPERR_SUCCESS;
}
...
netsnmp_table_data2_set_clone_row(netsnmp_table_data2row *row)
{
    ...
    if (data) {
        for (newrowdata =
             (netsnmp_table_data2_set_storage **) &(newrow->data); data;
             newrowdata = &((*newrowdata)->next), data = data->next) {

            memdup((u_char **) newrowdata, (u_char *) data,
                   sizeof(netsnmp_table_data2_set_storage));
            if (!*newrowdata) {
                netsnmp_table_dataset2_delete_row(newrow);
                return NULL;
            }
    ...
    (netsnmp_table_data2_set_storage *defrow)
{
    ...
    for (; defrow; defrow = defrow->next) {
        netsnmp_set_data2_row_column(row, defrow->column, defrow->type,
                               defrow->data.voidp, defrow->data_len);
        if (defrow->writable)
            netsnmp_mark_data2_row_column_writable(row, defrow->column, 1);
...
newrow_stash   *
netsnmp_table_data2_set_create_newrowstash
    (netsnmp_table_data2_set     *datatable,
     netsnmp_table_request_info *table_info)
{
    ...
    newrow = netsnmp_table_data2_set_create_row_from_defaults
...
int
netsnmp_table_data2_set_helper_handler(netsnmp_mib_handler *handler,
                                      ...
                                      netsnmp_request_info *requests)
{
    ...
    DEBUGMSGTL(("netsnmp_table_data2_set", "handler starting\n"));
    for (request = requests; request; request = request->next) {
        netsnmp_table_data2_set *datatable =
            (netsnmp_table_data2_set *) handler->myvoid;
        ...
        if (MODE_IS_SET(reqinfo->mode)) {
             ...
             */
            rc = snprintf(buf, sizeof(buf), "dataset2_row_stash:%s:",
            ...
            if (!newrowstash) {
                if (!row) {
                     ...
                     */
                    newrowstash = SNMP_MALLOC_TYPEDEF(newrow_stash);
                    newrow = netsnmp_table_data2_set_clone_row(row);
                    newrowstash->newrow = newrow;
                }
        ...
        data =
            netsnmp_table_data2_set_find_column(data, table_info->colnum);
		...
		if (newrowstash->created) {
                    netsnmp_table_dataset2_add_row(datatable, newrow);
                } else {
                    netsnmp_table_dataset2_replace_row(datatable,
                                                      row, newrow);
                }
            }
            /*
        ...
        case MODE_SET_UNDO:
            ...
            if (newrowstash->state != STATE_UNDO) {
                ...
                if (newrowstash->created) {
                    netsnmp_table_dataset2_remove_and_delete_row(datatable, newrow);
                } else {
                    netsnmp_table_dataset2_replace_row(datatable,
                                                      newrow, row);
                    netsnmp_table_dataset2_delete_row(newrow);
        ...
        case MODE_SET_COMMIT:
            if (newrowstash->state != STATE_COMMIT) {
                ...
                if (!newrowstash->created) {
                    netsnmp_table_dataset2_delete_row(row);
                }
...
/** registers a table_dataset2 so that the "add_row" snmpd.conf token
  ...
  */
void
netsnmp_register_auto_data2_table(netsnmp_table_data2_set *table_set2,
                                 char *registration_name)
{
    ...
    if (!tables)
        return;
    tables->table_set2 = table_set2;
    if (!registration_name) {
        registration_name = table_set2->table->name;
    }
...
NETSNMP_INLINE void
netsnmp_table_dataset2_add_index(netsnmp_table_data2_set *table, u_char type)
{
    if (!table)
        return;
 ...
 */
NETSNMP_INLINE netsnmp_table_data2_set_storage *
netsnmp_table_dataset2_delete_data2(netsnmp_table_data2_set_storage *data)
{
    netsnmp_table_data2_set_storage *nextPtr = NULL;
    if (data) {
        nextPtr = data->next;
        SNMP_FREE(data->data.voidp);
    }
...
NETSNMP_INLINE void
netsnmp_table_dataset2_delete_all_data2(netsnmp_table_data2_set_storage *data)
{

    while (data) {
        data = netsnmp_table_dataset2_delete_data2(data);
    }
}
 ...
 *  this is a wrapper around calling netsnmp_table_set2_add_default_row
 *  repeatedly for you.
 */
void
#if HAVE_STDARG_H
netsnmp_table_set2_multi_add_default_row(netsnmp_table_data2_set *tset, ...)
#else
netsnmp_table_set2_multi_add_default_row(va_dcl
    )
     va_dcl
#endif
{
...
#else
    netsnmp_table_data2_set *tset;

    va_start(debugargs);
    tset = va_arg(debugargs, netsnmp_table_data2_set *);
#endif
    ...
    while ((column = va_arg(debugargs, unsigned int)) != 0) {
        ...
        netsnmp_table_set2_add_default_row(tset, column, type, writable,
                                          data, data_len);
    }
...
/** adds multiple indexes to a table_dataset2 helper object.
...
netsnmp_table_set2_add_indexes(netsnmp_table_data2_set *tset,
                              ...)
#else
netsnmp_table_set2_add_indexes(va_alist)
     va_dcl
#endif
{
    va_list         debugargs;
    ...
    while ((type = va_arg(debugargs, int)) != 0) {
        netsnmp_table_dataset2_add_index(tset, (u_char)type);
    }
...
int
netsnmp_table_dataset2_num_rows(netsnmp_table_data2_set *table)
{
    if (!table)
        return 0;
...
int
netsnmp_table_set2_num_rows(netsnmp_table_data2_set *table)
{
 return netsnmp_table_set2_num_rows(table);
}
