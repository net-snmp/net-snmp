...
void init_logmatch(void)
{
  ...
  struct variable2 logmatch_table[] = {
    ...
    {LOGMATCH_FILENAME,   ASN_OCTET_STR, RONLY, var_logmatch_table, 1, {3}},
    {LOGMATCH_REGEX,      ASN_OCTET_STR, RONLY, var_logmatch_table, 1, {4}},
    {LOGMATCH_GLOBALCTR,  ASN_COUNTER,   RONLY, var_logmatch_table, 1, {5}},
    {LOGMATCH_GLOBALCNT,  ASN_INTEGER,   RONLY, var_logmatch_table, 1, {6}},
    {LOGMATCH_CURRENTCTR, ASN_COUNTER,   RONLY, var_logmatch_table, 1, {7}},
    {LOGMATCH_CURRENTCNT, ASN_INTEGER,   RONLY, var_logmatch_table, 1, {8}},
...
/* Define the OID pointer to the top of the mib tree that we're
  ...
  REGISTER_MIB("ucd-snmp/logmatch", logmatch_info,  variable2, logmatch_info_oid);
  REGISTER_MIB("ucd-snmp/logmatch", logmatch_table, variable2, logmatch_variables_oid);

  snmpd_register_config_handler("logmatch", logmatch_parse_config, logmatch_free_config,
...
void logmatch_parse_config(const char *token, char* cptr)
{
...
void updateLogmatch(int iindex)
{
    ...
    sprintf( perfilename, "%s/snmpd_logmatch_%s.pos", PERSISTENT_DIRECTORY, logmatchTable[iindex].name);
    ...
    if( stat(logmatchTable[iindex].filename, &sb) == 0) {
       ...
       if( logmatchTable[iindex].logfile = fopen(logmatchTable[iindex].filename, "r")) {
          ...
          result = fseek(logmatchTable[iindex].logfile, logmatchTable[iindex].currentFilePosition, SEEK_SET);

          if( result || toobig || (errno == EINVAL) || feof(logmatchTable[iindex].logfile) )  {
...
void updateLogmatch_Scheduled(unsigned int registrationNumber,  struct logmatchstat *logmatchtable) {
...
u_char *var_logmatch_table(struct variable *vp,
        oid *name,
        size_t *length,
        int exact,
        size_t *var_len,
        WriteMethod **write_method)
{
  ...
  if( vp->magic == LOGMATCH_INFO ) {
      ...
      if (header_simple_table(vp, name, length, exact, var_len, write_method, logmatchCount))
          return(NULL);
  }
  ...
  case LOGMATCH_MSG:
      ...
      regerror( logmatch->myRegexError, &(logmatch->regexBuffer), message, sizeof(message));
