...
void init_vmstat_hpux(void) 
{
  ...
  if (snmp_alarm_register(5, NULL, update_stats, NULL) == 0)
    {
      snmp_log(LOG_WARNING, "vmstat_hpux (init): snmp_alarm_register failed.\n");
    }  
  ...
  if (snmp_alarm_register(POLL_INTERVAL, SA_REPEAT, update_stats, NULL) == 0)
    {
      snmp_log(LOG_ERR, "vmstat_hpux (init): snmp_alarm_register failed, cannot service requests.\n");
    }  
...
static int take_snapshot(struct cpu_stat_snapshot *css)
{
  ...
  if ((css->css_time == 0) || (current_time > css->css_time + 2))
  {
      ...
      if (pstat_getdynamic(&psd, sizeof(psd), (size_t)1, 0) != -1) {
	   ...
	   for (i = 0; i < CPUSTATES; i++) {
	       css->css_cpu[i]	= (unsigned long long) psd.psd_cpu_time[i];
               DEBUGMSGTL(("take_snapshot","*** Time for CPU state %d: %d\n",i,psd.psd_cpu_time[i]));
           }
           psp = (struct pst_processor *)malloc(css->css_cpus * sizeof(*psp));
           if (pstat_getprocessor(psp, sizeof(*psp), css->css_cpus, 0) != -1) {
                int i;
                ...
                snmp_log(LOG_ERR, "vmstat_hpux (take_snapshot): pstat_getprocessor failed!\n");
      }
      else
           snmp_log(LOG_ERR, "vmstat_hpux (take_snapshot): pstat_getdynamic failed!\n");
      ...
      if (pstat_getvminfo(&psv, sizeof(psv), (size_t)1, 0) != -1) {
           ...
           snmp_log(LOG_ERR, "vmstat_hpux (take_snapshot): pstat_getvminfo failed!\n");
...
static void update_stats(unsigned int registrationNumber, void *clientarg)
{
  ...
  DEBUGMSGTL(("ucd-snmp/vmstat_hpux.c:update_stats","updating stats\n"));

  /* Take the current snapshot */
  if (take_snapshot(&snapshot[0]) == -1)
    {
      snmp_log(LOG_WARNING, "vmstat_hpux (update_stats): Something went wrong with take_snapshot.\n");
      return;
    }
  ...
  if (number_of_snapshots > 0)
    {
      ...
      if (snapshot[0].css_cpus != snapshot[1].css_cpus)
        {
          if (snapshot[0].css_cpus > snapshot[1].css_cpus)
            {
              snmp_log(LOG_NOTICE, "vmstat_hpux (update_stats): Cool ! Number of CPUs increased, must be hot-pluggable.\n");
            }
          else
            {
              snmp_log(LOG_NOTICE, "vmstat_hpux (update_stats): Lost at least one CPU, RIP.\n");
            }
	  ...
	  /* Try to get a new snapshot in five seconds so we can return s.th. useful */
          if (snmp_alarm_register(5, NULL, update_stats, NULL) == 0)
            {
              snmp_log(LOG_WARNING, "vmstat_hpux (update_stats): snmp_alarm_register failed.\n");
            }  
          return;
        }
      ...
      time_diff = (snapshot[0].css_time - snapshot[number_of_snapshots].css_time);

      DEBUGMSGTL(("ucd-snmp/vmstat_hpux.c:update_stats","time_diff: %lld\n", time_diff));
      ...
      swapin       = (unsigned int)((css_new->css_swapin - css_old->css_swapin) * pst.page_size / 1024 / time_diff ); 
      /* LINTED cast needed, really */
      swapout      = (unsigned int)((css_new->css_swapout - css_old->css_swapout) * pst.page_size / 1024 / time_diff); 
      /* LINTED cast needed, really */
      blocks_read  = (unsigned int)((css_new->css_blocks_read - css_old->css_blocks_read) / time_diff); 
      /* LINTED cast needed, really */
      blocks_write = (unsigned int)((css_new->css_blocks_write - css_old->css_blocks_write) / time_diff); 
      /* LINTED cast needed, really */
      interrupts   = (unsigned int)((css_new->css_interrupts - css_old->css_interrupts) / time_diff); 
      /* LINTED cast needed, really */
      context_sw   = (unsigned int)((css_new->css_context_sw - css_old->css_context_sw) / time_diff); 
      ...
      /* LINTED has to be 'long' */
      cpu_perc[CPU_SYSTEM] = (long) ((((css_new->css_cpu[CP_SYS] - css_old->css_cpu[CP_SYS])
                                     + (css_new->css_cpu[CP_WAIT]   - css_old->css_cpu[CP_WAIT])) 
                                            ...
                                            WriteMethod **write_method)
{
  ...
  case CPURAWUSER:
    ...
    /* LINTED has to be 'long' */
    long_ret = (long)(raw_values.css_cpu[CP_USER] / raw_values.css_cpus);
    return((u_char *) (&long_ret));
  case CPURAWNICE:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
    long_ret = (long)(raw_values.css_cpu[CP_NICE] / raw_values.css_cpus);
    return((u_char *) (&long_ret));
  case CPURAWSYSTEM:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
    long_ret = (long)((raw_values.css_cpu[CP_SYS] + raw_values.css_cpu[CP_WAIT]) / raw_values.css_cpus);
    return((u_char *) (&long_ret));
  case CPURAWIDLE:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
    long_ret = (long)(raw_values.css_cpu[CP_IDLE] / raw_values.css_cpus);
    return((u_char *) (&long_ret));    
  case CPURAWWAIT:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
    long_ret = (long)(raw_values.css_cpu[CP_WAIT] / raw_values.css_cpus);
    return((u_char *) (&long_ret));    
  case CPURAWKERNEL:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
    long_ret = (long)(raw_values.css_cpu[CP_SYS] / raw_values.css_cpus);
    return((u_char *) (&long_ret));    
  case IORAWSENT:
    take_snapshot(&raw_values);
    /* LINTED has to be 'long' */
  ...
  case IORAWRECEIVE:
    ...
    */
  default:
    snmp_log(LOG_ERR,"vmstat_hpux: Error in request, no match found.\n");
  }
  return(NULL);
