...
oid mteObjectsTable_variables_oid[] = { 1,3,6,1,2,1,88,1,3,1 };
...
#define   MTEOBJECTSIDWILDCARD  6
  { MTEOBJECTSIDWILDCARD, ASN_INTEGER   , RWRITE, var_mteObjectsTable, 2, { 1,4 } },
#define   MTEOBJECTSENTRYSTATUS  7
  { MTEOBJECTSENTRYSTATUS, ASN_INTEGER   , RWRITE, var_mteObjectsTable, 2, { 1,5 } },
...
parse_mteObjectsTable(const char *token, char *line) {
  size_t tmpint;
  struct mteObjectsTable_data *StorageTmp = SNMP_MALLOC_STRUCT(mteObjectsTable_data);
  ...
  line = read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->mteObjectsName, &StorageTmp->mteObjectsNameLen);
  if (StorageTmp->mteObjectsName == NULL) {
    config_perror("invalid specification for mteObjectsName");
    return;
  }

  line = read_config_read_data(ASN_UNSIGNED, line, &StorageTmp->mteObjectsIndex, &tmpint);

  line = read_config_read_data(ASN_OBJECT_ID, line, &StorageTmp->mteObjectsID, &StorageTmp->mteObjectsIDLen);
  if (StorageTmp->mteObjectsID == NULL) {
    config_perror("invalid specification for mteObjectsID");
    return;
  }

  line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->mteObjectsIDWildcard, &tmpint);

  line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->mteObjectsEntryStatus, &tmpint);
 ...
 */
int
store_mteObjectsTable(int majorID, int minorID, void *serverarg, void *clientarg) {
...
struct mteObjectsTable_data *StorageTmp = NULL;
  ...
  DEBUGMSGTL(("mteObjectsTable", "var_mteObjectsTable: Entering...  \n"));
  /* 
            ...
            size_t    name_len)
{
  ...
  DEBUGMSGTL(("mteObjectsTable", "write_mteObjectsID entering action=%d...  \n", action));
  if ((StorageTmp =
  ...
  switch ( action ) {
        ...
        case ACTION:
             ...
             memdup((u_char **) &StorageTmp->mteObjectsID, var_val, var_val_len);
            ...
            size_t    name_len)
{
  ...
  DEBUGMSGTL(("mteObjectsTable", "write_mteObjectsIDWildcard entering action=%d...  \n", action));
  if ((StorageTmp =
       header_complex(mteObjectsTableStorage, NULL,
                      &name[sizeof(mteObjectsTable_variables_oid)/sizeof(oid) + 3 - 1], 
                      &newlen, 1, NULL, NULL)) == NULL)
  ...
  /* stage one: test validity */
    ...
    /* XXX: interaction with row storage type needed */
  	  ...
  	  snmp_varlist_add_variable(&vars, NULL, 0, ASN_UNSIGNED, NULL, 0); /* mteObjectsIndex */
            ...
            if (header_complex_parse_oid(&(name[sizeof(mteObjectsTable_variables_oid)/sizeof(oid)+2]), newlen,
        ...
        case COMMIT:
          ...
          if (StorageDel != NULL) {
            ...
            if (StorageTmp && StorageTmp->mteObjectsEntryStatus == RS_CREATEANDGO) {
                StorageTmp->mteObjectsEntryStatus = RS_ACTIVE;
            } else if (StorageTmp &&
                       StorageTmp->mteObjectsEntryStatus == RS_CREATEANDWAIT) {
...
mte_add_objects(netsnmp_variable_list *vars, struct mteTriggerTable_data *item,
    ...
    while(hcptr &&
        ...
        /* copy in the suffix */
        memcpy(theoid, node->mteObjectsID, node->mteObjectsIDLen * sizeof(oid));
