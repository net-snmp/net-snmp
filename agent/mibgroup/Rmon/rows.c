 ...
 *
 * Revision 1.1.2.1  2001/04/16 14:47:30  alex
...
static void rowapi_too_long_creation_callback (unsigned int clientreg,
                                               void *clientarg)
{
  RMON_ENTRY_T* eptr;
  TABLE_DEFINTION_T* table_ptr;
  ...
  if (RMON1_ENTRY_VALID != eptr->status) {
    ag_trace ("row #%d in %s was under creation more then %ld sec.",
              eptr->ctrl_index, table_ptr->name, (long) MAX_CREATION_TIME);
    rowapi_delete (eptr);   
  } else {
...
int ROWAPI_new (TABLE_DEFINTION_T* table_ptr, u_long ctrl_index)
{
  ...
  if (table_ptr->max_number_of_entries > 0 &&
      table_ptr->current_number_of_entries >= table_ptr->max_number_of_entries)
    return -1;
...
{
  table_ptr->name =                     name;
  if (! table_ptr->name) table_ptr->name = "Unknown";
...
int ROWAPI_action_check (TABLE_DEFINTION_T* table_ptr, u_long ctrl_index)
{
  ...
  if (RMON1_ENTRY_UNDER_CREATION != eptr->status) {
    ...
    if (eptr->new_owner && 
        (! eptr->owner || strncmp (eptr->new_owner, eptr->owner, MAX_OWNERSTRING))) {
      ag_trace ("invalid owner string in ROWAPI_action_check");
      ag_trace ("eptr->new_owner=%p eptr->owner=%p", eptr->new_owner, eptr->owner);
      return SNMP_ERR_BADVALUE;
    }
  }
  ...
  switch (eptr->new_status) { /* this status we want to set */
    ...
    case RMON1_ENTRY_VALID:
      ...
      if (RMON1_ENTRY_UNDER_CREATION != eptr->status) {
        ag_trace ("Validate %s: entry %ld has wrong status %d", table_ptr->name, (long) ctrl_index, (int)  eptr->status);
        return SNMP_ERR_BADVALUE;
      }
                    ...
                    TABLE_DEFINTION_T* table_ptr, size_t entry_size)
{
  ...
  if (action == *prev_action) return SNMP_ERR_NOERROR; /* I want to process it only once ! */
  *prev_action = action;
                     ...
                     void *entry_ptr)
{
  ...
  if (ctrl_indx)
    data_index = (vp->namelen + 1 >= *length) ? 0 : name[vp->namelen + 1];
  else
    data_index = 0;
  ...
  if (exact) {
    ...
    if (hdr) {
      ...
      if (bptr && bptr->data_index <= data_index) bptr = NULL;
