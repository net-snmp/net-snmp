...
typedef struct {
  ...
  ALARM_TYPE_T		startup_type; /* RISING | FALLING | BOTH */
...
static TABLE_DEFINTION_T* table_ptr = &AlarmCtrlTable;

#if 0 /* KUKU */
static u_long kuku_sum = 0, kuku_cnt = 0;
#endif

/* find & enjoy it in event.c */
extern int event_api_send_alarm (u_char is_rising,
                          u_long alarm_index,
                          u_long event_index,
                          oid* alarmed_var,
...
fetch_var_val (oid *name, size_t namelen, u_long* new_value)
{
  ...
  memcpy (called_var.name, tree_ptr->name, tree_ptr->namelen * sizeof(oid));
  ...
  for (iii = 0; iii < tree_ptr->variables_len; iii++) {
    if (s_var_ptr->namelen) {
      if (0 >= snmp_oidtree_compare(name + tree_ptr->namelen,
                          ...
                          s_var_ptr->name,
                          s_var_ptr->namelen)) {
        memcpy (called_var.name + tree_ptr->namelen, s_var_ptr->name, s_var_ptr->namelen * sizeof(oid));
        ...
        access = (*(s_var_ptr->findVar)) (&called_var, name, &namelen, 1,
                                   &var_len, &write_method);
        if (access &&
            snmp_oid_compare (name, namelen,
                              tree_ptr->end, tree_ptr->end_len) > 0) {
        ...
        if (access) {
          ...
          switch (called_var.type) {
            ...
            case ASN_COUNTER64:
              break;
            default:
              ag_trace ("invalid type: %d", (int) called_var.type);
    ...
    s_var_ptr =(struct variable *)((char *)s_var_ptr + tree_ptr->variables_width);
  }
...
alarm_check_var (unsigned int clientreg, void *clientarg)
{
  ...
  if (ALARM_FALLING != body->prev_alarm &&
      body->value <= body->falling_threshold &&
      SNMP_ERR_NOERROR == event_api_send_alarm (0, hdr_ptr->ctrl_index,
...
int alarm_Validate (RMON_ENTRY_T* eptr)
{
...
int alarm_Activate (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  int ierr;

#if 0 /* KUKU */
  kuku_sum = 0; kuku_cnt = 0;
#endif
  ierr = fetch_var_val (body->var_name.objid,
                        body->var_name.length,
                        &body->last_abs_value);
...
int alarm_Deactivate (RMON_ENTRY_T* eptr)
{
  ...
  ag_trace ("kuku_sum=%ld kuku_cnt=%ld sp=%ld",
            (long) kuku_sum, (long) kuku_cnt, (long) (kuku_sum / kuku_cnt));
#endif
  return 0;
}
      ...
      switch (leaf_id) {
        ...
        case IDalarmVariable:
          ...
          if (RMON1_ENTRY_UNDER_CREATION != hdr->status &&
              snmp_oid_compare (cloned_body->var_name.objid, cloned_body->var_name.length,
                                body->var_name.objid, body->var_name.length))
            return SNMP_ERR_BADVALUE;
          break;
...
struct variable7 oidalarmVariables[] = {
  ...
  {IDalarmStartupAlarm,      ASN_INTEGER,  RWRITE,var_alarmEntry, 3,{1,1,6}},
  {IDalarmRisingThreshold,   ASN_INTEGER,  RWRITE,var_alarmEntry, 3,{1,1,7}},
  {IDalarmFallingThreshold,  ASN_INTEGER,  RWRITE,var_alarmEntry, 3,{1,1,8}},
  {IDalarmRisingEventIndex,  ASN_INTEGER,  RWRITE,var_alarmEntry, 3,{1,1,9}},
  {IDalarmFallingEventIndex, ASN_INTEGER,  RWRITE,var_alarmEntry, 3,{1,1,10}},
...
void init_alarm(void)
{
  ...
  ROWAPI_init_table (&AlarmCtrlTable, "Alarm", 0,
                     &alarm_Create,
                     NULL, /* &alarm_Clone, */
                     NULL, /* &alarm_Delete, */
                     &alarm_Validate,
