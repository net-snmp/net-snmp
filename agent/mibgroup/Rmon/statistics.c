 ...
 *
 * Revision 1.1.2.1  2001/04/20 13:46:27  tali
 * Begin of the history. Actually, there is only Ethernet
 * statistics for alarm & event debugging
 */
...
static TABLE_DEFINTION_T* table_ptr = &StatCtrlTable;

/* Control Table RowApi Callbacks */

int stat_Create (RMON_ENTRY_T* eptr)
{/* create the body: alloc it and set defaults */
  CRTL_ENTRY_T* body;
  static VAR_OID_T data_src_if_index_1 = { 11, {1,3,6,1,2,1,2,2,1,1,1} };
...
int stat_Validate (RMON_ENTRY_T* eptr)
{
...
int stat_Activate (RMON_ENTRY_T* eptr)
{
...
int stat_Copy (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  CRTL_ENTRY_T* clone = (CRTL_ENTRY_T*) eptr->tmp;

  if (snmp_oid_compare (clone->data_source.objid, clone->data_source.length,
...
int stat_Deactivate (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  memset (&body->eth, 0, sizeof (ETH_STATS_T));
  return 0;
 ...
 ***************************************************/
u_char *var_etherStats2Entry (struct variable *vp, oid *name, size_t *length,
		...
		oid *name, size_t name_len)
{
  ...
  switch (action) {
    ...
    case COMMIT:
    default:
      snmp_status =  ROWAPI_do_another_action (name, etherStatsEntryFirstIndexBegin,
                        action, &prev_action,
      ...
      switch (leaf_id) {
        case Leaf_etherStatsDataSource:
          ...
          if (RMON1_ENTRY_UNDER_CREATION != hdr->status &&
              snmp_oid_compare (cloned_body->data_source.objid, cloned_body->data_source.length,
                                body->data_source.objid, body->data_source.length))
 ...
 ***************************************************/
u_char *var_etherStatsEntry (struct variable *vp, oid *name, size_t *length,
			int exact, size_t *var_len, WriteMethod **write_method)
{
  static long         long_return;
  static CRTL_ENTRY_T theEntry;
...
int add_statistics_entry (int ctrl_index, int ifIndex)
{
  int ierr;
...
struct variable7 oidstatisticsVariables[] = {
  {IDetherStatsIndex,ASN_INTEGER,RONLY,var_etherStatsEntry, 3,{1,1,1}},
  {IDetherStatsDataSource,ASN_OBJECT_ID,RWRITE,var_etherStatsEntry, 3,{1,1,2}},
  {IDetherStatsDropEvents,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,3}},
  {IDetherStatsOctets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,4}},
  {IDetherStatsPkts,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,5}},
  {IDetherStatsBroadcastPkts,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,6}},
  {IDetherStatsMulticastPkts,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,7}},
  {IDetherStatsCRCAlignErrors,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,8}},
  {IDetherStatsUndersizePkts,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,9}},
  {IDetherStatsOversizePkts,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,10}},
  {IDetherStatsFragments,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,11}},
  {IDetherStatsJabbers,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,12}},
  {IDetherStatsCollisions,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,13}},
  {IDetherStatsPkts64Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,14}},
  {IDetherStatsPkts65to127Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,15}},
  {IDetherStatsPkts128to255Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,16}},
  {IDetherStatsPkts256to511Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,17}},
  {IDetherStatsPkts512to1023Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,18}},
  {IDetherStatsPkts1024to1518Octets,ASN_COUNTER,RONLY,var_etherStatsEntry, 3,{1,1,19}},
  {IDetherStatsOwner,ASN_OCTET_STR,RWRITE,var_etherStatsEntry, 3,{1,1,20}},
  {IDetherStatsStatus,ASN_INTEGER  ,RWRITE,var_etherStatsEntry, 3,{1,1,21}},
  {IDetherStatsDroppedFrames,ASN_COUNTER,RONLY,var_etherStats2Entry, 3,{4,1,1}},
  {IDetherStatsCreateTime,ASN_TIMETICKS,RONLY,var_etherStats2Entry, 3,{4,1,2}},
};
...
{
  REGISTER_MIB(MIB_DESCR, oidstatisticsVariables, variable7, oidstatisticsVariablesOid);

  ROWAPI_init_table (&StatCtrlTable, MIB_DESCR, 0,
                     &stat_Create,
                     NULL, /* &stat_Clone, */
                     NULL, /* &stat_Delete, */
                     &stat_Validate,
