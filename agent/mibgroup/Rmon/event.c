...
int event_Create (RMON_ENTRY_T* eptr)
...
int event_Clone (RMON_ENTRY_T* eptr)
...
int event_Copy (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  CRTL_ENTRY_T* clone = (CRTL_ENTRY_T*) eptr->tmp;
...
int event_Delete (RMON_ENTRY_T* eptr)
{
...
int event_Activate (RMON_ENTRY_T* eptr)
{ /* init logTable */
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
...
int event_Deactivate (RMON_ENTRY_T* eptr)
{/* free logTable */
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  ...
  *var_len = sizeof(long); /* default */
  ...
  switch(vp->magic) {
    ...
    case EVENTSTATUS:
      ...
      return (unsigned char *) &long_ret;
    default:
      ag_trace ("EventControlTable: unknown vp->magic=%d", (int) vp->magic);
      ERROR_MSG("");
  }
  return NULL;
}

static SCROLLER_T* event_extract_scroller (void* v_body)
{
                 ...
                 u_int32_t sample_type)
{
  ...
  static oid alarm_index_oid[] = {1,3,6,1,2,1,16,3,1,1,1};
  static oid alarmed_var_oid[] = {1,3,6,1,2,1,16,3,1,1,3};
  ...
  top = oa_bind_var (top, alarmed_var, ASN_OBJECT_ID, sizeof (oid) * alarmed_var_length,
                     alarmed_var_oid, OID_LENGTH(alarmed_var_oid));
...
int event_api_send_alarm (u_char is_rising,
                          u_long alarm_index,
                          u_long event_index,
                          oid* alarmed_var,
                          size_t alarmed_var_length,
                          ...
                          char* alarm_descr)
{
  ...
  ag_trace ("event_api_send_alarm(%d,%d,%d,'%s')",
            (int) is_rising, (int) alarm_index, (int) event_index, alarm_descr);
#endif
  ...
  if (EVENT_TRAP == evptr->event_type || EVENT_LOG_AND_TRAP == evptr->event_type) {
    event_send_trap (evptr, is_rising, alarm_index,
                     value, the_threshold,
                     alarmed_var, alarmed_var_length,
                     sample_type);
  }

  if (EVENT_LOG == evptr->event_type || EVENT_LOG_AND_TRAP == evptr->event_type) {
...
int add_event_entry (int ctrl_index,
                     char* event_description,
                     EVENT_TYPE_T event_type,
                     char* event_community)
{
...
oid eventTable_variables_oid[] = { 1,3,6,1,2,1,16,9,1 };
...
void init_event(void)
{
  ...
  ROWAPI_init_table (&EventCtrlTable, "Event", 0,
                     &event_Create,
                     &event_Clone,
                     &event_Delete,
                     NULL, /* &event_Validate, */
