...
static int
                getmib(int groupname, int subgroupname, void *statbuf, size_t size,
                       ...
                       size_t * length, int (*comp) (void *, void *),
                       void *arg);
...
extern          "C" {
    ...
    void            init_kernel_sunos5(void) {
        ...
        if (creg == 0) {
            creg =
                snmp_alarm_register(period, SA_REPEAT,
                                    kernel_sunos5_cache_age,
                                    (void *) period);
            DEBUGMSGTL(("kernel_sunos5",
                        "registered alarm %d with period %ds\n", creg,
                        period));
        }
    }
    ...
    int             getKstatInt(const char *classname,
        ...
        default:
            DEBUGMSGTL(("kernel_sunos5",
                        "non-integer type in kstat data: %s %s %s %d\n",
    ...
    int
         ...
         */
        if ((ks =
             kstat_lookup(ksc, module_name, instance, statname)) == NULL) {
            ret = -10;
            goto Return;        /* kstat errors */
        }
        if (kstat_read(ksc, ks, NULL) <= 0) {
        ...
        for (i = 0, d = KSTAT_NAMED_PTR(ks); i < ks->ks_ndata; i++, d++) {
            ...
            if (strcmp(d->name, varname) == 0) {
                ...
                default:
                    DEBUGMSGTL(("kernel_sunos5",
                                "Unknown type in kstat data: %s %s %d\n",
                   ...
                   req_e req_type, int (*comp) (void *, void *), void *arg)
    {
        ...
        DEBUGMSGTL(("kernel_sunos5",
                    "... cache_valid %d time %ld ttl %d now %ld\n",
        ...
        if (cache_valid) {
            ...
            if (req_type == GET_NEXT) {
                ...
                result =
                    getentry(req_type, cachep->cache_addr,
    ...
    static          found_e
        getentry(req_e req_type, void *bufaddr, size_t len,
                 size_t entrysize, void *resp, int (*comp) (void *,
                                                            void *),
                 void *arg) {
            ...
            do {
                ...
                case 0:
                    if (req_type == GET_NEXT && result == NEED_NEXT)
                        ...
                        req_type = GET_FIRST;
                    result =
                        getentry(req_type, (void *) strbuf.buf, strbuf.len,
    ...
    static int
        ...
        getif(mib2_ifEntry_t * ifbuf, size_t size, req_e req_type,
              mib2_ifEntry_t * resp, size_t * length, int (*comp) (void *,
                                                                   void *),
              void *arg) {
        int             i, ret, idx = 1;
        int             ifsd;
      ...
      Again:
        for (i = 0, ifp = (mib2_ifEntry_t *) ifbuf, ifrp = ifconf.ifc_req;
            ...
            if (getKstat(ifrp->ifr_name, "ifspeed", &ifp->ifSpeed) == 0
                ...
                if (ifp->ifSpeed < 10000)
                    ifp->ifSpeed *= 1000000;
            } else if (getKstat(ifrp->ifr_name, "ifSpeed", &ifp->ifSpeed)
                       == 0) {
                /*
                 * this is good 
                 */
	    ...
	    case 'd': /* dmfe -- found on netra X1 */
            ...
            if (!strchr(ifrp->ifr_name, ':')) {
                Counter         l_tmp;
                if (getKstat
                    (ifrp->ifr_name, "ipackets",
                     &ifp->ifInUcastPkts) < 0) {
                    ret = -1;
                    goto Return;
                }
                if (getKstat(ifrp->ifr_name, "rbytes", &ifp->ifInOctets) <
                    0)
                    ifp->ifInOctets = ifp->ifInUcastPkts * 308; /* XXX */
                if (getKstat
                    (ifrp->ifr_name, "opackets",
                     &ifp->ifOutUcastPkts) < 0) {
                    ret = -1;
                    goto Return;
                }
                if (getKstat(ifrp->ifr_name, "obytes", &ifp->ifOutOctets) <
                    0)
                    ifp->ifOutOctets = ifp->ifOutUcastPkts * 308;       /* XXX */
                if (ifp->ifType == 24)  /* Loopback */
                    continue;
                if (getKstat(ifrp->ifr_name, "ierrors", &ifp->ifInErrors) <
                    0) {
                    ret = -1;
                    goto Return;
                }
                if (getKstat(ifrp->ifr_name, "oerrors", &ifp->ifOutErrors)
                    < 0) {
                    ret = -1;
                    goto Return;
                }
                if (getKstat
                    (ifrp->ifr_name, "brdcstrcv",
                     &ifp->ifInNUcastPkts) == 0
                    && getKstat(ifrp->ifr_name, "multircv", &l_tmp) == 0)
                    ifp->ifInNUcastPkts += l_tmp;
                if (getKstat
                    (ifrp->ifr_name, "brdcstxmt",
                     &ifp->ifOutNUcastPkts) == 0
            ...
            if (getMibstat
                (MIB_IP_NET, &Media, sizeof(mib2_ipNetToMediaEntry_t),
        ...
        if ((req_type == GET_NEXT) && (result == NEED_NEXT))
            ...
            req_type = GET_FIRST;
        result =
            getentry(req_type, (void *) ifbuf, size,
                     sizeof(mib2_ifEntry_t), (void *) resp, comp, arg);
        if ((result != FOUND) && (i == nentries)
            && ((char *) ifrp <
                (char *) ifconf.ifc_buf + ifconf.ifc_len)) {
            /*
    ...
    static int
        ...
        struct sockaddr_in *s =
            (struct sockaddr_in *) &(((struct ifreq *) ifrp)->ifr_addr);
