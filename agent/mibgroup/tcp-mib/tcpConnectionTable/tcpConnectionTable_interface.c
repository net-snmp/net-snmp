...
void
tcpConnectionTable_dirty_set(int status)
{
                                    ...
                                    netsnmp_request_info *requests)
{
    ...
    if (MFD_SUCCESS != rc) {
         ...
         */
        DEBUGMSGTL(("internal:tcpConnectionTable", "error %d from "
                                     ...
                                     netsnmp_request_info *requests)
{
     ...
     */
    if ((SNMP_ERR_NOERROR == agtreq_info->asp->status) &&
        (NULL !=
         (rowreq_ctx = netsnmp_container_table_row_extract(requests)))) {
        if (rowreq_ctx->rowreq_flags & MFD_ROW_DELETED) {
            CONTAINER_REMOVE(tcpConnectionTable_if_ctx.container,
                             rowreq_ctx);
            tcpConnectionTable_release_rowreq_ctx(rowreq_ctx);
        } else {
 ...
 */
static int
_mfd_tcpConnectionTable_object_lookup(netsnmp_mib_handler *handler,
...
int
_mfd_tcpConnectionTable_check_objects(netsnmp_mib_handler *handler,
                                      netsnmp_handler_registration
                                      *reginfo,
                                      netsnmp_agent_request_info
                                      *agtreq_info,
 ...
 */
static int
_mfd_tcpConnectionTable_check_dependencies(netsnmp_mib_handler *handler,
                                           netsnmp_handler_registration
                                           *reginfo,
                                           netsnmp_agent_request_info
                                           *agtreq_info,
                                           netsnmp_request_info *requests)
{
    ...
    if (rc) {
        DEBUGMSGTL(("verbose:tcpConnectionTable:mfd", "error %d from "
    ...
    if (MFD_SUCCESS != rc) {
        ...
        for (; requests; requests = requests->next) {
            ...
            rc = _tcpConnectionTable_undo_setup_column(rowreq_ctx,
                                                       tri->colnum);
            if (MFD_SUCCESS != rc) {
                DEBUGMSGTL(("verbose:tcpConnectionTable:mfd",
                            "error %d from "
