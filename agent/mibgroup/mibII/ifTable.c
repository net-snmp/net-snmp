...
init_ifTable(void)
{
    ...
    netsnmp_inject_handler( reginfo,
		    netsnmp_get_cache_handler(IF_STATS_CACHE_TIMEOUT,
...
#define ENTENDED_IF_TABLE
     ...
     */
    DEBUGMSGTL(("mibII/ifTable", "Initialising Interface Extension Table\n"));
    table_info = SNMP_MALLOC_TYPEDEF(netsnmp_table_registration_info);
    if (!table_info) {
        return;
    }
                     ...
                     netsnmp_iterator_info *data)
{
     ...
     */
    idx = index;
    snmp_set_var_value(idx, (u_char*)&(entry->index), sizeof(entry->index));
                 ...
                 netsnmp_request_info         *requests)
{
    ...
    case MODE_GET:
        for (request=requests; request; request=request->next) {
            ...
            entry = (netsnmp_ifentry *)netsnmp_extract_iterator_context(request);
            if (!entry)
                continue;
	        ...
	        snmp_set_var_typed_value(requestvb, ASN_COUNTER,
                                         (u_char *)&entry->if_iunknown_protos,
                                             sizeof(entry->if_iunknown_protos));
                break;

            case COLUMN_IFOUTOCTETS:
                if ( entry->flags & NETSNMP_IF_FLAGS_HAS_BYTES )
	            snmp_set_var_typed_value(requestvb, ASN_COUNTER,
                                         (u_char *)&entry->if_obytes.low,
