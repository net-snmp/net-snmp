...
#ifdef USE_ASN_SHORT_LEN
/* patch by Josh Wilmes (jwilmes@cisco.com)
 * modified by Mike Slifcak (mslifcak@iss.net) to work in 4.0 release.
 *
 * This patch will slow the packet constructing method,
 * as each packet is essentially built twice :
 * Once to form the contents, and a second time to reduce
 * to "short form" length encodings whenever possible.
 *
 * SNMPv3 packets are built with no special consideration for short lengths.
 * Only SNMPv1 and SNMPv2c packets are built with the shortest lengths.
 */
	   ...
	   size_t *out_length)
{
    ...
    int pdu_len, varbind_len;
    long version;
    u_char tmp_packet[PACKET_LENGTH];
   ...
   /* now calculate how big each part of the packet is going to be, counting
      for the variable-length "size" field */
   
   varbind_len = h3 - h2;

   if (varbind_len < 0x80)      varbind_len += 2;
   else if (varbind_len < 0xFF) varbind_len += 3;  
   else                         varbind_len += 4;

   pdu_len = h2 - h1 + varbind_len;

   if (pdu_len < 0x80)      pdu_len += 2;
   else if (pdu_len < 0xFF) pdu_len += 3; 
   else                     pdu_len += 4;
   ...
   /* insert variable bindings sequence */
   memmove(cp, h2, h3 - h2);
   cp += (h3 - h2);   
		     ...
		     void *cb_data)
{
    ...
    if (pdu == NULL) {
        session->s_snmp_errno = 0; /* XX what error ? */
        return 0;
    }
