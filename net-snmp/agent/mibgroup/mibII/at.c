...
static void ARP_Scan_Init (void)
{
	...
	if (sysctl(mib, 6, NULL, &needed, NULL, 0) < 0)
        ...
        else if ((at = malloc(needed ? needed : 1)) == NULL)
            snmp_log_perror("malloc");
        else if (sysctl(mib, 6, at, &needed, NULL, 0) < 0)
    ...
    case RESERVE1:    
      switch(var){
        ...
        case IPMEDIAPHYSADDRESS:
          ...
          if (var_val_len != 6) {
            snmp_log(LOG_ERR, "not correct ipAddress length: %d", var_val_len);
            return SNMP_ERR_WRONGLENGTH;
          }
          break;
        default:
        ...
        case IPMEDIANETADDRESS:
          ...
          if(!create_flag){
            if(SetIpNetEntry(arp_row) != NO_ERROR){
              ...
              temp_row.dwType = 2;
              if(SetIpNetEntry(&temp_row) != NO_ERROR){
                snmp_log(LOG_ERR, "Failed in ACTION, while deleting old row \n");
                retval = SNMP_ERR_COMMITFAILED;
              }
            } 
          }
    ...
    case COMMIT:
      ...
      if(create_flag) {
        if(arp_row->dwPhysAddrLen != 0){
          ...
          snmp_log(LOG_ERR, "Can't create new entry without physical address\n");
