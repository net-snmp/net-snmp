 ...
 *
 * Revision 1.1.2.1  2001/04/16 14:47:27  alex
                      ...
                      oid *name, size_t name_len)
{
  ...
  switch (action) {
    ...
    case COMMIT:
    default:
      return ROWAPI_do_another_action (name, historyControlEntryFirstIndexBegin,
                        action, &prev_action,
      ...
      switch (leaf_id) {
        case Leaf_historyControlDataSource:
          ...
          if (RMON1_ENTRY_UNDER_CREATION != hdr->status &&
            ...
            ag_trace ("can't change historyControlDataSource - not Creation");
...
#if 0
          if (RMON1_ENTRY_UNDER_CREATION != hdr->status &&
              cloned_body->scrlr.data_requested != body->scrlr.data_requested)
            return SNMP_ERR_BADVALUE;
#endif
          break;
                ...
                WriteMethod **write_method)
{
  ...
  *var_len = sizeof(long); /* default */
  ...
  switch(vp->magic) {
    ...
    default:
      ag_trace ("HistoryControlTable: unknown vp->magic=%d", (int) vp->magic);
      ERROR_MSG("");
  }
  return NULL;
}
...
static void history_get_backet (unsigned int clientreg,
                                void *clientarg)
{
  ...
  bptr->utilization = bptr->EthData.octets * 8 + bptr->EthData.packets * (96 + 64);
...
int history_Create (RMON_ENTRY_T* eptr)
{/* create the body: alloc it and set defaults */
  ...
  ROWDATAAPI_init (&body->scrlr, HIST_DEF_BUCK_REQ,
                   MAX_BUCKETS_IN_CRTL_ENTRY,
                   sizeof (DATA_ENTRY_T), NULL);
                            
  return 0;
}

int history_Validate (RMON_ENTRY_T* eptr)
{
  /* T.B.D. (system dependent) check valid inteface in body->data_source; */
  return 0;
}

int history_Activate (RMON_ENTRY_T* eptr)
{
  ...
  SYSTEM_get_eth_statistics (&body->data_source, &body->previous_bucket.EthData);
  body->previous_bucket.start_interval =  AGUTIL_sys_up_time ();
...
int history_Deactivate (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
...
int history_Copy (RMON_ENTRY_T* eptr)
{
  CRTL_ENTRY_T* body = (CRTL_ENTRY_T*) eptr->body;
  ...
  if (body->interval != clone->interval) {
    if (RMON1_ENTRY_VALID == eptr->status) {
      snmp_alarm_unregister (body->timer_id);
      body->timer_id = snmp_alarm_register (clone->interval, SA_REPEAT,
                                        history_get_backet, eptr);
    }
...
static SCROLLER_T* history_extract_scroller (void* v_body)
{
                ...
                WriteMethod **write_method)
{
  ...
  *var_len = sizeof(long); /* default */
  ...
  switch(vp->magic) {
    ...
    default:
        ag_trace ("etherHistoryTable: unknown vp->magic=%d", (int) vp->magic);
        ERROR_MSG("");
  }
  return NULL;
}
...
int add_hist_entry (int ctrl_index, int ifIndex,
...
oid historyControlTable_variables_oid[] = { 1,3,6,1,2,1,16,2,1 };

struct variable2 historyControlTable_variables[] = {
/*  magic number        , variable type, ro/rw , callback fn  ,           L, oidsuffix */
  {CTRL_INDEX,            ASN_INTEGER  , RONLY , var_historyControlTable, 2, { 1,1 } },
  {CTRL_DATASOURCE,       ASN_OBJECT_ID, RWRITE, var_historyControlTable, 2, { 1,2 } },
  {CTRL_BUCKETSREQUESTED, ASN_INTEGER  , RWRITE, var_historyControlTable, 2, { 1,3 } },
  {CTRL_BUCKETSGRANTED,   ASN_INTEGER  , RONLY , var_historyControlTable, 2, { 1,4 } },
  {CTRL_INTERVAL,         ASN_INTEGER  , RWRITE, var_historyControlTable, 2, { 1,5 } },
  {CTRL_OWNER ,           ASN_OCTET_STR, RWRITE, var_historyControlTable, 2, { 1,6 } },
  {CTRL_STATUS,           ASN_INTEGER  , RWRITE, var_historyControlTable, 2, { 1,7 } },

};

oid etherHistoryTable_variables_oid[] = { 1,3,6,1,2,1,16,2,2 };

struct variable2 etherHistoryTable_variables[] = {
/*  magic number     , variable type , ro/rw , callback fn  ,        L, oidsuffix */
  {DATA_INDEX,          ASN_INTEGER  , RONLY, var_etherHistoryTable, 2, { 1,1 } },
  {DATA_SAMPLEINDEX,    ASN_INTEGER  , RONLY, var_etherHistoryTable, 2, { 1,2 } },
  {DATA_INTERVALSTART,  ASN_TIMETICKS, RONLY, var_etherHistoryTable, 2, { 1,3 } },
  {DATA_DROPEVENTS,     ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,4 } },
  {DATA_OCTETS,         ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,5 } },
  {DATA_PKTS,           ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,6 } },
  {DATA_BROADCASTPKTS,  ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,7 } },
  {DATA_MULTICASTPKTS,  ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,8 } },
  {DATA_CRCALIGNERRORS, ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,9 } },
  {DATA_UNDERSIZEPKTS,  ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,10 } },
  {DATA_OVERSIZEPKTS,   ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,11 } },
  {DATA_FRAGMENTS,      ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,12 } },
  {DATA_JABBERS,        ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,13 } },
  {DATA_COLLISIONS,     ASN_COUNTER  , RONLY, var_etherHistoryTable, 2, { 1,14 } },
  {DATA_UTILIZATION,    ASN_INTEGER  , RONLY, var_etherHistoryTable, 2, { 1,15 } },
...
void init_history(void)
{
  REGISTER_MIB("historyControlTable", historyControlTable_variables, variable2,
               historyControlTable_variables_oid);
  REGISTER_MIB("etherHistoryTable", etherHistoryTable_variables, variable2,
               etherHistoryTable_variables_oid);

  ROWAPI_init_table (&HistoryCtrlTable, "History", 0, 
                     &history_Create,
                     NULL, /* &history_Clone, */
                     NULL, /* &history_Delete, */
                     &history_Validate,
