...
snmpNotifyFilterTable_commit(snmpNotifyFilterTable_rowreq_ctx * rowreq_ctx)
{
     ...
     */
    if (save_flags & COLUMN_SNMPNOTIFYFILTERMASK_FLAG) {
        save_flags &= ~COLUMN_SNMPNOTIFYFILTERMASK_FLAG;        /* clear snmpNotifyFilterMask */
        /*
         * TODO:482:o: |-> commit column snmpNotifyFilterMask.
         */
        rc = -1;
        if (-1 == rc) {
            snmp_log(LOG_ERR,
                     "snmpNotifyFilterTable column snmpNotifyFilterMask commit failed\n");
        } else {
            /*
             * set flag, in case we need to undo snmpNotifyFilterMask
             */
            rowreq_ctx->column_set_flags |=
                COLUMN_SNMPNOTIFYFILTERMASK_FLAG;
        }
    }

    if (save_flags & COLUMN_SNMPNOTIFYFILTERTYPE_FLAG) {
        save_flags &= ~COLUMN_SNMPNOTIFYFILTERTYPE_FLAG;        /* clear snmpNotifyFilterType */
        /*
         * TODO:482:o: |-> commit column snmpNotifyFilterType.
         */
        rc = -1;
        if (-1 == rc) {
            snmp_log(LOG_ERR,
                     "snmpNotifyFilterTable column snmpNotifyFilterType commit failed\n");
        } else {
            /*
             * set flag, in case we need to undo snmpNotifyFilterType
             */
            rowreq_ctx->column_set_flags |=
                COLUMN_SNMPNOTIFYFILTERTYPE_FLAG;
        }
    }

    if (save_flags & COLUMN_SNMPNOTIFYFILTERSTORAGETYPE_FLAG) {
        save_flags &= ~COLUMN_SNMPNOTIFYFILTERSTORAGETYPE_FLAG; /* clear snmpNotifyFilterStorageType */
        /*
         * TODO:482:o: |-> commit column snmpNotifyFilterStorageType.
         */
        rc = -1;
        if (-1 == rc) {
            snmp_log(LOG_ERR,
                     "snmpNotifyFilterTable column snmpNotifyFilterStorageType commit failed\n");
        } else {
            /*
             * set flag, in case we need to undo snmpNotifyFilterStorageType
             */
            rowreq_ctx->column_set_flags |=
    ...
    if (save_flags & COLUMN_SNMPNOTIFYFILTERROWSTATUS_FLAG) {
        save_flags &= ~COLUMN_SNMPNOTIFYFILTERROWSTATUS_FLAG;   /* clear snmpNotifyFilterRowStatus */
        /*
         * TODO:482:o: |-> commit column snmpNotifyFilterRowStatus.
         */
        rc = -1;
        if (-1 == rc) {
            snmp_log(LOG_ERR,
                     "snmpNotifyFilterTable column snmpNotifyFilterRowStatus commit failed\n");
        } else {
            /*
             * set flag, in case we need to undo snmpNotifyFilterRowStatus
             */
            rowreq_ctx->column_set_flags |=
