 ...
 */
int
ipv4InterfaceTable_index_to_oid(netsnmp_index * oid_idx,
                                ipv4InterfaceTable_mib_index * mib_idx)
{
     ...
     */
    netsnmp_variable_list var_ipv4InterfaceIfIndex;

    /*
     * set up varbinds
     */
    memset(&var_ipv4InterfaceIfIndex, 0x00,
           sizeof(var_ipv4InterfaceIfIndex));
    var_ipv4InterfaceIfIndex.type = ASN_INTEGER;

    /*
     * chain temp index varbinds together
     */
    var_ipv4InterfaceIfIndex.next_variable = NULL;


    DEBUGMSGTL(("verbose:ipv4InterfaceTable:ipv4InterfaceTable_index_to_oid", "called\n"));

    /*
     * ipv4InterfaceIfIndex(1)/InterfaceIndex/ASN_INTEGER/long(long)//l/a/w/e/R/d/H 
     */
    snmp_set_var_value(&var_ipv4InterfaceIfIndex,
                       (u_char *) & mib_idx->ipv4InterfaceIfIndex,
                       sizeof(mib_idx->ipv4InterfaceIfIndex));


    err = build_oid_noalloc(oid_idx->oids, oid_idx->len, &oid_idx->len,
                            NULL, 0, &var_ipv4InterfaceIfIndex);
    if (err)
     ...
     */
    snmp_reset_var_buffers(&var_ipv4InterfaceIfIndex);

    return err;
}                               /* ipv4InterfaceTable_index_to_oid */

/**
 * extract ipv4InterfaceTable indexes from a netsnmp_index
 *
 * @retval SNMP_ERR_NOERROR  : no error
 * @retval SNMP_ERR_GENERR   : error
 */
int
ipv4InterfaceTable_index_from_oid(netsnmp_index * oid_idx,
                                  ipv4InterfaceTable_mib_index * mib_idx)
{
    int             err = SNMP_ERR_NOERROR;
    ...
    DEBUGMSGTL(("verbose:ipv4InterfaceTable:ipv4InterfaceTable_index_from_oid", "called\n"));

    /*
     * parse the oid into the individual index components
     */
    err = parse_oid_indexes(oid_idx->oids, oid_idx->len,
                            &var_ipv4InterfaceIfIndex);
    if (err == SNMP_ERR_NOERROR) {
        /*
         * copy out values
         */
        mib_idx->ipv4InterfaceIfIndex =
            *((long *) var_ipv4InterfaceIfIndex.val.string);
    ...
    return err;
}                               /* ipv4InterfaceTable_index_from_oid */
 ...
 *********************************************************************
 * @internal
 * allocate resources for a ipv4InterfaceTable_rowreq_ctx
 */
ipv4InterfaceTable_rowreq_ctx *
ipv4InterfaceTable_allocate_rowreq_ctx(void *user_init_ctx)
{
    ipv4InterfaceTable_rowreq_ctx *rowreq_ctx =
        SNMP_MALLOC_TYPEDEF(ipv4InterfaceTable_rowreq_ctx);

    DEBUGMSGTL(("internal:ipv4InterfaceTable:ipv4InterfaceTable_allocate_rowreq_ctx", "called\n"));
    ...
    rowreq_ctx->ipv4InterfaceTable_data_list = NULL;
    ...
    if (!(rowreq_ctx->rowreq_flags & MFD_ROW_DATA_FROM_USER)) {
        if (SNMPERR_SUCCESS !=
            ipv4InterfaceTable_rowreq_ctx_init(rowreq_ctx,
                                               user_init_ctx)) {
    ...
    return rowreq_ctx;
}                               /* ipv4InterfaceTable_allocate_rowreq_ctx */
 ...
 */
static int
_mfd_ipv4InterfaceTable_object_lookup(netsnmp_mib_handler *handler,
...
int
_mfd_ipv4InterfaceTable_check_objects(netsnmp_mib_handler *handler,
                                      netsnmp_handler_registration
                                      *reginfo,
                                      netsnmp_agent_request_info
                                      *agtreq_info,
 ...
 */
int
_mfd_ipv4InterfaceTable_irreversible_commit(netsnmp_mib_handler *handler,
                                            netsnmp_handler_registration
                                            *reginfo,
                                            netsnmp_agent_request_info
                                            *agtreq_info,
                                         ...
                                         mib_idx)
{
     ...
     * convert
     */
    rc = ipv4InterfaceTable_index_to_oid(&oid_idx, mib_idx);
    if (MFD_SUCCESS != rc)
        return NULL;
