...
char	       *snmp_unix_fmtaddr	(netsnmp_transport *t,
					 void *data, int len)
{
...
int		netsnmp_unix_close	(netsnmp_transport *t)
{
  ...
  if (t->sock >= 0) {
    ...
    if (sup != NULL) {
      if (sup->local) {
	DEBUGMSGTL(("netsnmp_unix_close", "server unlink(\"%s\")\n",
		    sup->server.sun_path));
	unlink(sup->server.sun_path);
      } else {
	DEBUGMSGTL(("netsnmp_unix_close", "client unlink(\"%s\")\n",
		    sup->client.sun_path));
...
int		netsnmp_unix_accept	(netsnmp_transport *t)
{
  ...
  if (t != NULL && t->sock >= 0) {
    ...
    if (newsock < 0) {
      DEBUGMSGTL(("netsnmp_unix_accept", "accept failed rc %d errno %d \"%s\"\n",
    ...
    DEBUGMSGTL(("netsnmp_unix_accept", "accept succeeded (farend %p len %d)\n",
		farend, farendlen));
    t->data = farend;
						 ...
						 int local)
{
  ...
  t->domain_length = sizeof(netsnmp_ucdSnmpUnixDomain)/sizeof(netsnmp_ucdSnmpUnixDomain[0]);
...
netsnmp_transport	*snmp_unix_create_tstring	(const char *string, int local)
{
...
netsnmp_transport	*snmp_unix_create_ostring	(const u_char *o, size_t o_len,
						 int local)
{
  struct sockaddr_un addr;
...
void		netsnmp_unix_ctor			(void)
{
  ...
  unixDomain.name_length = sizeof(netsnmp_ucdSnmpUnixDomain)/sizeof(oid);
