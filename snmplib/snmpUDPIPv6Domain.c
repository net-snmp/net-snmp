						 ...
						 int remote_port)
{
  ...
  if (inpeername != NULL) {
	...
	*cp = '\0'; 
	if ( *(cp + 1) == ':' ) {
	  if (atoi(cp + 2) != 0 && 
	      inet_pton(AF_INET6, peername + 1, (void *)&(addr->sin6_addr))) {
	  ...
	  if (inet_pton(AF_INET6, peername + 1, (void *)&(addr->sin6_addr))) {
	    DEBUGMSGTL(("netsnmp_sockaddr_in6", "IPv6 address with square brankets\n"));
	    addr->sin6_port = htons(SNMP_PORT);
	    goto resolved;
	  } 
	}
...
inet_addr_complement( int pf, void *src, void *dst){
  ...
  switch(pf){
  case PF_INET:
    ((struct in_addr *)dst)->s_addr = ~((struct in_addr *)src)->s_addr;
    break;
  case PF_INET6:
    for( i = 0 ; i < 16 ; i++ ){
      (*(uint8_t *)(&((struct in6_addr *)dst)->s6_addr[i])) = (~(*(uint8_t *)(&((struct in6_addr *)src)->s6_addr[i]))) & 0xff;
    }
    break;
  default:
    return -1; 
...
inet_addr_and( int pf, void *src1, void *src2, void *dst) {
  ...
  switch(pf){
  case PF_INET:
    ((struct in_addr *)dst)->s_addr = ((struct in_addr *)src1)->s_addr & ((struct in_addr *)src2)->s_addr ;
    break;

  case PF_INET6:
    for( i = 0 ; i < 16 ; i++ ){
      (*(uint8_t *)(&((struct in6_addr *)dst)->s6_addr[i])) = 
	(*(uint8_t *)(&((struct in6_addr *)src1)->s6_addr[i])) & (*(uint8_t *)(&((struct in6_addr *)src2)->s6_addr[i]));
    }
    break;
  default:
    return -1;
...
inet_addrs_consistence (int pf, void *net, void *mask ) {
  ...
  case PF_INET:
    ...
    if (inet_addr_complement(PF_INET, (struct in_addr *)mask, &tmp->sin_addr) != 0){
    ...
    if (inet_addr_and(PF_INET, (struct in_addr *)net, &tmp->sin_addr, &dst->sin_addr) != 0){
  ...
  case PF_INET6:
    ...
    if (inet_addr_complement(PF_INET6, (struct in6_addr *)mask, &tmp6->sin6_addr) != 0){
      config_perror("Fail in function of inet_addr_complement()");
      free(tmp6);
      return -1;
    }
    ...
    if (inet_addr_and(PF_INET6, (struct in6_addr *)net, &tmp6->sin6_addr, &dst6->sin6_addr)){
      config_perror("Fail in function of inet_addr_and()");
      free(dst6);
      free(tmp6);
      return -1;
...
masked_address_are_equal(int af, struct sockaddr_storage *from, 
  ...
  case PF_INET6:
    ...
    if ( IN6_ARE_ADDR_EQUAL( &((struct sockaddr_in6 *)&ss)->sin6_addr,
			     &((struct sockaddr_in6 *)network)->sin6_addr ) == 1 ){
      return 0;
    } else {
      return -1;
    }
...
void            memmove_com2Sec6Entry           (com2Sec6Entry *c,
						 char *secName,
						 char *community,
						 ...
						 char *param)
{
  ...
  char *cp = NULL, *strnetwork = NULL, *strmask = NULL, *strnetmask = NULL; 
  ...
  if ((strcmp(source, "default") == 0) || (strcmp(source, "::") == 0)) {
    ...
    if (strmask != NULL && strnetwork != NULL){
      DEBUGMSGTL(("snmp_udp6_parse_security", "<\"%s\", %s/%s> => \"%s\"\n",
		  community, strnetwork, strmask, secName));
      free(strmask);
      free(strnetwork);
    } else {
      DEBUGMSGTL(("snmp_udp6_parse_security", "Couldn't allocate enough memory\n"));
    }
	...
	if (strchr(strmask, ':')) {
	  ...
	  if (inet_make_mask_addr(PF_INET6, &mask.sin6_addr, atoi(strmask)) != 0) {
	    config_perror("bad mask");
	    return;
      ...
      if (inet_addrs_consistence(PF_INET6, &net.sin6_addr, &mask.sin6_addr) != 0){
	...
	/*  Everything is okay.  Copy the parameters to the structure allocated
	    above and add it to END of the list.  */
	DEBUGMSGTL(("snmp_udp6_parse_security", "<\"%s\", %s> => \"%s\"\n",
		    community, hbuf, secName));
	memmove_com2Sec6Entry(e, secName, community, net, mask); 
	if (com2Sec6ListLast != NULL) {
	  com2Sec6ListLast->next = e;
	  com2Sec6ListLast = e;
...
void		snmp_udp6_com2Sec6List_free		(void)
{
					 ...
					 int community_len, char **secName)
{
  ...
  if (opaque == NULL || olength != sizeof(struct sockaddr_in6) || from->sin6_family != PF_INET6){
    DEBUGMSGTL(("netsnmp_udp6_getSecName", "no IPv6 source address in PDU?\n"));
    if (secName != NULL) {
      *secName = NULL;
    }
    return 1;
  ...
  for (c = com2Sec6List; c != NULL; c = c->next) {
    DEBUGMSGTL(("netsnmp_udp6_getSecName", "compare <\"%s\", 0x%032/0x%032x>",
	...
	(masked_address_are_equal(from->sin6_family, 
				  ...
				  (struct sockaddr_storage *)&c->network) == 0)) {
