...
/**
 * create probe for engineID
 *
 * Designed to create PDU for asynchronous or synchronous USM probe, SNMP_FLAGS_DONT_PROBE must be set to 
 * disable internal synchronous probing, when response is received and
 * all callbacks have executed the rest of PDUs can be sent as usual.
 */
netsnmp_pdu
*snmpv3_probe_usm_pdu_create(void)
{
	netsnmp_pdu	*pdu;
...
_build_initial_pdu_packet(struct session_list *slp, netsnmp_pdu *pdu, int bulk)
{
    ...
    /*
     * check to see if we need to perform v3 engineID probe. Call post probe hook to 
     * create user from information in a session even if SNMP_FLAGS_DONT_PROBE is set,
     * as this may indicate that probe was already sent by other means for example asynchronously.
     */
    if ((pdu->version == SNMP_VERSION_3) &&
        (pdu->flags & UCD_MSG_FLAG_EXPECT_RESPONSE)) {
        int rc;
